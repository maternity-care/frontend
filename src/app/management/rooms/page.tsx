...
import type { RoomDeleteTarget } from "./components/RoomDeleteModal";
...
import type { FacilityOption } from "./components/room-form.shared";
...
function renderStatus(status: RoomStatus) {
...
function isEmptyRoomResult(error: unknown) {
  ...
  return (
    response?.status === 404 ||
    normalizedMessage.includes("không tìm thấy phòng") ||
    normalizedMessage.includes("không có phòng") ||
    normalizedMessage.includes("no rooms found") ||
...
function ClinicRoomManagementContent() {
  ...
  const authUser = user as unknown as RoomAccessUser | null;
  ...
  const canViewAllFacilities = roomAccess.canViewAllFacilities;
  const canManageRooms = roomAccess.canManage;
  const scopedFacilityId = roomAccess.facilityId;
  const requestedFacilityFilter = searchParams.get("facilityId") || undefined;
  ...
  const managedFacilities = useMemo(
    () =>
      canManageRooms
        ...
        : [],
    [canManageRooms, facilities, scopedFacilityId],
  );

  function canManageRoom(room: ClinicRoom) {
    return Boolean(
      canManageRooms &&
      scopedFacilityId &&
  ...
  const [roomTypeLookupReloadKey, setRoomTypeLookupReloadKey] = useState(0);

  const [editingRoom, setEditingRoom] = useState<ClinicRoom | null>(null);
  const [detailRoomId, setDetailRoomId] = useState<string | null>(null);
  const [detailInitialRoom, setDetailInitialRoom] = useState<ClinicRoom | null>(
    null,
  );
  const [deleteTarget, setDeleteTarget] = useState<RoomDeleteTarget | null>(
    null,
  );
  const [bulkOpen, setBulkOpen] = useState(false);
  const [roomTypesOpen, setRoomTypesOpen] = useState(false);
  const [statusAction, setStatusAction] = useState<RoomStatusAction>(null);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      const request = getRooms({
        ...params,
        facilityId: canViewAllFacilities ? facilityFilter : scopedFacilityId,
      });
      ...
      void request
        ...
        .catch((loadError) => {
          ...
          setRooms([]);
          setTotalRooms(0);
          if (isEmptyRoomResult(loadError)) {
            setError(null);
            return;
          }
  ...
  const tableRooms = useMemo(() => {
    ...
    const backendReturnedFullList =
      totalRooms > 0 && rooms.length >= totalRooms;

    if (backendReturnedFullList) {
      const offset = (currentPage - 1) * pageSize;

      return rooms.slice(offset, offset + pageSize);
    }

    return rooms.slice(0, pageSize);
  }, [currentPage, pageSize, rooms, totalRooms]);

  const roomById = useMemo(
    () => new Map(tableRooms.map((room) => [room.id, room])),
    [tableRooms],
  );
  ...
  function openStatusAction(action: RoomStatusAction) {
    if (!action || !canManageRoom(action.room)) {
      return;
    }
  ...
  function updateRoomInState(updatedRoom: ClinicRoom) {
    setRooms((current) =>
  ...
  async function handleConfirmStatusAction() {
    if (!statusAction || !canManageRoom(statusAction.room)) {
      return;
    }
  ...
  function openDetail(roomId: string) {
    ...
    if (
      !room ||
      (!canViewAllFacilities && String(room.facilityId) !== scopedFacilityId)
    ) {
      return;
    }
  ...
  function handleDeleted(deletedIds: string[]) {
    ...
    if (tableRooms.length <= deletedIds.length && currentPage > 1) {
  ...
  const columns: ColumnsType<ClinicRoom> = [
    ...
    },
    {
      ...
      render: (roomTypeName: string) => (
        <Tag color="blue">{roomTypeName || "Chưa cập nhật"}</Tag>
      ),
    },
    {
      title: "Cơ sở",
      ...
      render: (_value, room) => (
        <div className="min-w-0">
          <Text strong className="block truncate">
            {room.facilityName || "Chưa cập nhật"}
          </Text>

          <Text type="secondary" className="block truncate text-xs">
            {room.facilityCode || room.facilityId}
          </Text>
        </div>
      ),
    },
    {
      ...
      width: 90,
      align: "center",
      render: (floor: string) => floor || "Chưa cập nhật",
    },
    {
      title: "Trạng thái",
      dataIndex: "status",
      width: 130,
      align: "center",
      render: (status: RoomStatus) => renderStatus(status),
    },
    {
      title: "Thao tác",
      width: canManageRooms ? 185 : 80,
      align: "center",
      render: (_value, room) => {
    ...
    <AdminLayout roles={["super_admin", "admin"]} permissions={["user.view"]}>
      ...
      <div>
        ...
        <p className="mb-0 text-sm text-slate-500">
          Quản lý phòng, loại phòng và dữ liệu phòng theo từng cơ sở.
        </p>
      </div>

      <Modal
        open={canManageRooms && Boolean(statusAction)}
        title={
      ...
      >
        {statusAction?.mode === "suspend" ? (
          ...
          <Text>
            Phòng sẽ được chuyển về trạng thái hoạt động. Các ca trực đã bị hủy
            trước đó sẽ không tự khôi phục.
          </Text>
        )}
      </Modal>
      ...
      <div className="mt-6 flex flex-col gap-5">
        ...
        <Card className="border-slate-200 bg-white">
          <div
            className={`grid items-end gap-3 sm:grid-cols-2 ${
              canViewAllFacilities ? "lg:grid-cols-6" : "lg:grid-cols-5"
            }`}
          >
            <Input
              allowClear
            ...
            <Input
              ...
              placeholder="Tầng"
              onChange={(event) => {
                setFloorFilter(event.target.value || undefined);
                setCurrentPage(1);
              }}
            />
        ...
        <Card
          ...
          extra={
            <Space wrap>
              <Badge count={totalRooms} showZero color="#0f766e" />

              {canManageRooms ? (
                <>
                  <Button
                    icon={<Shapes className="h-4 w-4" />}
                    onClick={() => setRoomTypesOpen(true)}
                  >
                    Loại phòng
                  </Button>
      ...
      <RoomDetailModal
        ...
        canManage={detailInitialRoom ? canManageRoom(detailInitialRoom) : false}
      ...
      {canManageRooms ? (
        <RoomTypeManagementModal
          ...
          onChanged={() => {
            setRoomTypeLookupReloadKey((current) => current + 1);
            refreshRooms();
          }}
        />
      ) : null}
